...
Boundary_CONUS2 <- Boundary_CONUS |>
  st_as_sf() |> rmapshaper::ms_simplify(keep = 0.05, method = "vis", keep_shapes = TRUE)
...
## Through the process of simpifying my data I was able to remove 10,715 points (11,292-577). The consequences of doing this computationally is there is fewer data now available, making it easier to manage but there might be different statistics if all the points were included.
```
...
Dams <- function(arg1, arg2) {
  ggplot(data = arg1) +
    ...
    theme_void() +
    labs(title = arg2, caption = paste("Number of dams:", sum(arg1$n, na.rm = TRUE)))}
```
